...
load("//allen/programs/celltypes/workgroups/rnaseqanalysis/V1_ALM/process/WGCNA_result_anterograde/V1.ref.49.df.rda")
load("//allen/programs/celltypes/workgroups/rnaseqanalysis/V1_ALM/process/WGCNA_result_anterograde/V1.ref.97.df.rda")
...
new_to_tasic_expanded <- new_to_tasic %>%
  repeat_rows("Rf.map.num")

# make_plot_nodes converts group_nodes to rectangles for plotting with geom_rect()
n2t_nodes <- make_plot_nodes(make_group_nodes(new_to_tasic_expanded, c("dendcluster","final")))
...
new_to_tasic_river <- build_river_plot(new_to_tasic_expanded, 
                                       c("dendcluster","final"), 
...
ggsave("randomforest_new_to_tasic_river.pdf", new_to_tasic_river, height = 24, width = 6)

# L4 Only
new_to_tasic_expanded <- new_to_tasic_expanded %>%
  filter(dendcluster_id == 56)
...
# make_plot_nodes converts group_nodes to rectangles for plotting with geom_rect()
t2n_nodes <- make_plot_nodes(make_group_nodes(tasic_to_new_expanded, c("final","dendcluster")),
                             pad = 0.2)
...
tasic_to_new_river <- build_river_plot(tasic_to_new_expanded, 
                                       c("final","dendcluster"), 
                                       fill_group = "final",
                                       pad = 0.2) +
...
# make_plot_nodes converts group_nodes to rectangles for plotting with geom_rect()
tasic_to_new_expanded <- tasic_to_new_expanded %>%
  filter(final_id %in% 26:28)
t2n_nodes <- make_plot_nodes(make_group_nodes(tasic_to_new_expanded, c("final","dendcluster")),
                             pad = 0.2)
...
ggsave("randomforest_tasic_to_new_river_L4.pdf", tasic_to_new_river, height = 4, width = 6)
